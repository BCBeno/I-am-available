...
export default function OwnerAvailabilityDetailsScreen({ route, navigation, setLoggedInUser, user }) {
  ...
  const daysOfWeek = [
    ...
    { label: 'W', full: 'Wednesday' }, { label: 'T', full: 'Thursday' }, { label: 'F', full: 'Friday' }, { label: 'S', full: 'Saturday' }
  ];
      ...
      <ScrollView contentContainerStyle={styles.container}>
          ...
          <View style={styles.iconRow}>
            ...
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', flex: 1 }}>
              {daysOfWeek.map((dayObj) => {
                ...
                return (
                  <View key={dayObj.full} style={[styles.dayButton, isActive && styles.activeDayButton]}>
        ...
        <Text style={styles.readOnlyField} selectable>
          {availability.complement.split(/(https?:\/\/[^\s]+)/g).map((part, index) =>
            part.match(/https?:\/\/[^\s]+/) ? (
              <Text key={index} style={{ color: '#7C0152', textDecorationLine: 'underline' }} onPress={() => Linking.openURL(part)}>
                {part}
              </Text>
            ) : (
