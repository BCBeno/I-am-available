...
export const deleteRoleData = async (roleHashtag, userHashtag) => {
    try {
        ...
        // 4.1 Update user document to remove references to these groups
        ...
        if (userSnap.exists()) {
            ...
            const updatedGroups = userGroups.filter(
                (g) => !ownedGroupIds.has(g.groupReference.replace('/groups/', ''))
            );
